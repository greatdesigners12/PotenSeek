...
fun PieChart(
    ...
    progress: List<Float>,
    colors: List<Color> = listOf(orderOne,orderTwo,orderThree, orderFour, orderFive),
    ...
    return Path().apply {
        drawArc(
            ...
            style = if (isDonut) Stroke(
                width = if (isActive) 180f else 150f,
            ) else Fill,
...
fun PieChartPrev() {
    PieChart(
        progress = listOf(70f, 10f, 10f, 10f)
    )
}